...
| `/ctx [轮数]` | 查看当前对话最近几轮的上下文用量，默认 5 轮，范围 1–100 |
...
文字命令均不按单条字符数或回复总字符数截断内容；目录分页和查询条数限制仍然生效。
...
`/ctx` 和 `/ctx-pic` 默认显示最近 5 轮，`/ctx 10` 或 `/ctx-pic 10` 显示最近 10 轮。命令仅限管理员，在当前会话回复；群内统计对群成员可见。采集覆盖启用本插件的会话中普通成员与管理员触发的内置 Agent 对话，查询不会调用模型。

命令回复只显示轮次数、简短时间和用量数值，不附带对话 ID、来源行或统计口径说明；单位 tokens 在标题中统一标注。

一轮指一次主 Agent 运行。工具循环可能请求模型多次，展示的是该轮**最后一次请求**的用量，不是多次请求的累计消耗。按接口实际提供的字段展示输入、输出、缓存读取、缓存写入、思考和合计；只有合计时只显示合计，没有用量时明确显示“未提供用量”。优先读取 OpenAI Chat Completions、Responses、Gemini 和 Anthropic 的原始用量，原始报告不可用时使用 AstrBot 提供的标准化用量；标准化对象默认的零值无法证明该字段由接口返回，因此不展示这些零值。原始报告明确返回的 0 会保留。

输入 token 是该次请求实际报告的上下文规模。输入已经包含缓存读取，输出包含单列的思考 token，不应再次相加。输入＋输出可以作为下一轮占用的参考，但不是精确的实时上下文大小：下一轮的动态系统提示词、工具定义、消息裁剪和压缩都可能改变它。插件不使用字符数估算或累计账单用量冒充上下文占用。

统计从安装启用后开始，每个对话最多保留最近 100 轮，使用插件 KV 持久化，重启和重载后仍可查询。保存时间、轮次、数值及用于关联的消息哈希，不额外保存对话正文。记录按会话及对话 ID 隔离，`/new` 和切换对话不会混用统计，切回旧对话可查看其仍在上下文中的记录。

成功 `/reset` 后清除对应统计；权限不足或执行失败不会删除。查询及新请求开始时还会对照实际保存的上下文，清理已清空、删除、裁剪或压缩掉的轮次。WebUI 或其他插件清空上下文时，在下次查询或请求时完成清理。只展示已经保存且仍能与当前上下文对应的轮次，正在生成的回复不会被当作已完成记录。清空后重新从第 1 轮编号。

启用前各轮明细无法补回；没有逐轮记录但 AstrBot 保存了最近一次合计时，单独展示这个合计并注明不能还原明细。外部 Agent 或其他插件绕过主 Agent 钩子、没有本地对话记录时，无法提供逐轮统计。

## 图片输出

`logs-pic`、`chatlog-pic` 和 `ctx-pic` 共用对应文字版的数据查询、权限、脱敏和条数限制。`logs-pic` 仅限管理员私聊，`chatlog-pic` 和 `ctx-pic` 在当前会话回复，群内图片对群成员可见。空记录、非法参数等仍用简短文字说明。
...
`ctx-pic` 使用表格对齐轮次、时间与各项用量，只展示所选轮次中有数据的统计列。单轮缺失值以“—”标识，实际返回的 0 正常显示；所有轮次都没有用量时显示“未提供用量”。已有对话只有最近一次合计时，仅显示合计并注明“无明细”。不附带对话 ID、来源或大段说明。

图片宽度为 1200 像素。日志与 JSON 长行自动换行，每页最多 56 行正文，沿完整显示行分页；用量表格每页最多 20 轮，保持行完整，每页重复表头。所有页面渲染完成后依次交给 AstrBot 标准回复流程发送，页脚注明页码；图片保存在内存中，不留日志或对话截图文件。单次渲染超过 120 秒会明确报错，不发送不完整的渲染结果。插件串行执行渲染任务，结束后关闭浏览器。
